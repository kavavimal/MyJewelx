...
const CartSidebar = () => {
  ...
  return (
    <>
      ...
      <div
        ref={sidebarRef}
        className={`fixed overflow-auto top-0 right-0 z-[101] h-full w-96 bg-white shadow-lg transition-transform transform ${
      ...
      >
        <div className="p-4 ">
          ...
          {carts && carts?.cartItems?.length > 0 ? (
            <>
              <ul role="list" className="p-0 divide-y divide-gray-200">
                {carts?.cartItems?.map((cartItem) => {
                  return (
                    <li className="flex py-3 p-0" key={cartItem.id}>
