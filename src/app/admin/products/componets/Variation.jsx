...
const Variation = ({
    ...
    if (
        ...
        variation?.productAttributeValues?.length > 0
    ) {
    ...
    const getMetalPrice = () => {
        if (
            ...
            const mapstring = attributeSelected
                .replace(/\s/g, '')
                .toLowerCase();
    ...
    const variatonValidationSchema = Yup.object().shape({
        ...
        gross_weight: Yup.number()
            .min(
                Yup.ref('net_weight'),
                'Gross weight should be more than net weight'
            )
                    ...
                    const response = await post('/api/variation', {
                        ...
                        productAttributeValue_id:
                            variation.productAttributeValues
                                ...
                                .join(','),
                        stock_status:
                            values.stock_status === 'in_stock' ? true : false,
                        selling_price: total,
                    });
        ...
        if (
            ...
            if (Object.keys(pricing).includes(mapstring)) {
                ...
                if (formik.values?.net_weight) {
                    metalPrice =
                        metalPrice * parseFloat(formik.values?.net_weight || 0);
                }
            }
            ...
            if (formik.values?.isPriceFixed) {
                ...
                if (makingCharge === 'Per Gram On Net Weight') {
                    ...
                    price =
                        price +
                            (metalPrice * (parseFloat(chargeValue) || 0)) /
                                100 || 0;
                    setTotalMakingCharge(
            ...
            if (
                isDiscount &&
                discountType === 'Per(%) On Metal Rate On Karat'
            ) {
                price =
                    price -
                        (metalPrice * (parseFloat(discountValue) || 0)) / 100 ||
                    0;
                setAppliedDiscout(
        ...
        if (isVariation) {
            try {
                ...
                if (otherCharges) {
                    ...
                    setIsGemstone((prev) =>
                        prev !== hasGemstone ? hasGemstone : prev
                    );
                    setGemstoneCharges((prev) =>
                        JSON.stringify(prev) !==
                        JSON.stringify(newGemstoneCharges)
                            ? newGemstoneCharges
                            : prev
                    );
                    setIsAdditionalCharges((prev) =>
                        prev !== hasAdditionalCharges
                        ...
                        <Option value="Grams">Grams</Option>
                                    ...
                                    <div className="pt-3">
                                        ...
                                        <Checkbox
                                            ...
                                            checked={isGemstone}
                                            onChange={(e) =>
                                                setIsGemstone(e.target.checked)
                                            }
                                        />
                                        {isGemstone && (
                                            <div>
