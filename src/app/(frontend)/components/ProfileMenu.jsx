...
export default function ProfileMenu({ placement }) {
  const { data: session, status } = useSession();
  const user = useUserStore((state) => state.user);
  const updateUser = useUserStore((state) => state.fetchUser);
  useEffect(() => {
    updateUser();
  }, [session]);
  return (
      ...
      {status === "loading" ? (
        ...
        <>
          {session ? (
            ...
            >
              <MenuHandler>
                <div className="relative cursor-pointer inline-flex items-center justify-center w-10 h-10 select-none overflow-hidden hover:opacity-80 bg-primary-200/25 rounded-full dark:bg-gray-600">
                  <span className="font-medium text-black uppercase">{`${session.user?.firstName[0]}${session?.user?.lastName[0]}`}</span>
                </div>
              </MenuHandler>
              <MenuList className="z-[999991]">
                {user?.status === UserStatus.ACTIVE &&
                  [AcountType.ADMIN, AcountType.VENDOR].includes(
                    user.role.role_name
                  ) && (
                ...
                {[AcountType.VENDOR, AcountType.CUSTOMER].includes(
                  user?.role?.role_name
                ) && (
                  <Link
                    href="/jewlex-on-demand/my-requests"
                    role="menuitem"
                    className="flex w-full cursor-pointer select-none items-center gap-2 rounded-md px-3 pt-[9px] pb-2 text-start leading-tight outline-none transition-all hover:bg-blue-gray-50 hover:bg-opacity-80 hover:text-blue-gray-900 focus:bg-blue-gray-50 focus:bg-opacity-80 focus:text-blue-gray-900 active:bg-blue-gray-50 active:bg-opacity-80 active:text-blue-gray-900"
                  >
                    <svg
                ...
                {[AcountType.CUSTOMER].includes(user?.role?.role_name) && (
                  <>
                    <Link
                      href="/support"
                      role="menuitem"
            ...
            <li>
              <Link href="/login" className="text-primary-200 font-emirates">
                Login
              </Link>
            </li>
          )}
