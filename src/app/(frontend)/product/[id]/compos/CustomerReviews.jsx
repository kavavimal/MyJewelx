...
const CustomerReviews = ({ reviews, product_id, user_id }) => {
  ...
  const handleReview = () => setReviewOpen(!reviewOpen);
  ...
  return (
    <>
      <div className="pb-[43px]">
        <div>
          <div className="flex items-center justify-between mb-5">
            <h3 className="text-2xl md:text-xl xl:text-2xl font-playfairdisplay font-semibold">
              Customer Reviews
            </h3>
            <div>
              <Button onClick={() => handleReview()} variant="outlined">
                {reviewOpen === false ? "Write Review" : "Cancel Review"}
              </Button>
            </div>
          </div>
          <div className="px-[15px] pt-5 pb-[26px] md:px-5 md:pt-[29px] md:pb-[31px] border border-blueGray-300">
            <div className="flex md:flex-row flex-col items-center justify-between">
              <div className="md:pl-4 md:pr-2 lg:pl-[60px] md:block flex items-center w-full md:w-[500px] border-b border-b-blueGray-300 md:border-b-0 md:border-r md:border-r-blueGray-300">
                <span className="block text-[56px] pr-[15px] md:pr-0  font-normal font-playfairdisplay">
                  {Math.floor(average)}
                </span>
                <div>
                  <div>
                  ...
                  <div className="flex flex-col mt-[5px] md:mt-[15px]">
                    <div className="inline-flex">
                      <p className="text-base inline font-light md:border-b md:border-b-blueGray-400 pb-2.5">
                        Total {reviews?.length ?? 0} customer reviews
                      </p>
                    </div>
                    <span className="md:block hidden text-secondary-100 pt-2.5 pb-[19px]">
                      {Math.floor(percent)}% customers recommended this product
                    </span>
                  </div>
                </div>
              </div>
              <div className="flex pl-3 flex-col md:pt-0 pt-5 w-full lg:w-auto gap-[15px]">
                <div className="flex gap-[15px] items-center">
                  <span className="text-secondary-100 w-full max-w-[90px] font-light">
                    5 Star reviews
                  </span>
                  <div className="h-[15px] bg-primary-100 w-full md:w-full lg:w-[432px] ">
                    <div
      ...
      <div
        className={`pb-[15px] ${
          reviewOpen ? "block" : "hidden"
        }  border-b border-blueGray-300`}
      >
        <div className="w-full md:w-[620px]">
          <ReviewForm user_id={user_id} product_id={product_id} />
        </div>
      </div>
    </>
  );
