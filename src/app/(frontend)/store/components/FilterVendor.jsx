"use client";
import { initFiltersData, useVendorStore } from "@/contexts/vendorStore";
import {
  Accordion,
  AccordionBody,
  AccordionHeader,
...
const FilterVendor = ({ filterdDatas }) => {
  const selectedFilters = useVendorStore((vendor) => vendor.filters);
  const setSelectedFilters = useVendorStore((vendor) => vendor.setFilters);
  ...
  const [isShowMoreSoldProducts, setIsShowMoreSoldProducts] = useState(false);
  const [isShowMoreLocations, setIsShowMoreLocations] = useState(false);
  const [isShowMore, setIsShowMore] = useState(false);
  const [openAccordions, setOpenAccordions] = useState([0, 1]);
  const [max, setMax] = useState(1999);
    ...
    if (isChecked) {
      filters = {
        ...
        locations: event.target.checked
          ? [...filters.locations, location.text]
          : [...filters.locations.filter((c) => c !== location.text)],
      };
      setSelectedFilters(filters);
    } else {
      filters = {
        ...filters,
        locations: filters.locations.filter((c) => c !== location.text),
      };
      setSelectedFilters(filters);
    }
  };
  ...
  const handleSoldProductsChange = (event, min, max) => {
    const isChecked = event.target.checked;
    let filters = selectedFilters;
    ...
    if (isChecked) {
      filters = {
        ...filters,
        soldProducts: [...filters.soldProducts, { min, max }],
      };
    } else {
      filters = {
        ...filters,
        soldProducts: filters.soldProducts.filter(
          (soldProduct) => soldProduct.min !== min || soldProduct.max !== max
        ),
      };
    }
  ...
  const handlefilterbylikes = (min, max) => {
    let filters = selectedFilters;
    filters = {
      ...filters,
      likes: { min, max },
  ...
  const LocationOptions = [
    { location_id: 1, text: "Sharjah" },
    { location_id: 2, text: "Dubai" },
    { location_id: 3, text: "india" },
    { location_id: 4, text: "Abu Dhabi" },
    { location_id: 5, text: "Fujairah" },
    { location_id: 6, text: "Bhavnagar" },
    { location_id: 7, text: "Deira" },
    { location_id: 8, text: "New York" },
    { location_id: 9, text: "Madhya Pradesh" },
    { location_id: 10, text: "Bihar" },
    { location_id: 11, text: "Chandigarh" },
    { location_id: 12, text: "London" },
    { location_id: 13, text: "Tokyo" },
    { location_id: 14, text: "San Andreas" },
    { location_id: 15, text: "San Fransisco" },
    { location_id: 16, text: "Kolkata" },
  ];

  const soldproductsOptions = [
    { soldproducts_id: 1, text: "50+", min: 50, max: 99 },
    { soldproducts_id: 2, text: "300+", min: 300, max: 499 },
    { soldproducts_id: 3, text: "100+", min: 100, max: 149 },
    { soldproducts_id: 4, text: "500+", min: 500, max: 699 },
    { soldproducts_id: 5, text: "200+", min: 200, max: 299 },
    { soldproducts_id: 6, text: "1000+", min: 700, max: 999 },
  ];
  ...
  return (
    <div>
      <div className="mb-4">
        <Button
          onClick={() => {
            setSelectedFilters(initFiltersData);
          }}
          variant="outlined"
      ...
      >
        <AccordionHeader
          ...
          onClick={() => {
            ...
            setMax(max === 2000 ? 1999 : 2000);
          }}
        >
          Filter by Likes
        </AccordionHeader>
        <AccordionBody className="pb-8">
          <div className="min-w-0 py-2 p-0 px-0 grid gap-3">
            <FilterLikes max={max} filterByLikes={handlefilterbylikes} />
          </div>
        </AccordionBody>
      </Accordion>
        ...
        <AccordionBody>
          <div className="min-w-0 py-2 p-0 px-0 grid  gap-3">
            {LocationOptions &&
              ...
              LocationOptions.slice(
                0,
                isShowMoreLocations ? LocationOptions.length : 3
              ).map((location) => {
                return (
                  <div className="p-0" key={location?.text}>
                    <label
                    ...
                    >
                      <Checkbox
                        value={location?.text}
                        onChange={(event) =>
                          handlelocationChange(event, location)
                        }
                        checked={selectedFilters?.locations?.includes(
                          location?.text
                        )} // Use 'includes' to check if the location is in selectedFilters.locations array
                        className="w-4 h-4"
                        labelProps={{ className: "pl-2" }}
                        id={location?.text}
                        ripple={false}
            ...
            {LocationOptions && LocationOptions.length > 3 && (
              <button
                variant="text"
                className="flex justify-start mt-4"
                onClick={() => setIsShowMoreLocations(!isShowMoreLocations)}
                ripple={false}
              >
                {isShowMoreLocations ? (
                  "Show Less"
      ...
      >
        ...
        >
          Filter by Sold Products
        </AccordionHeader>
        <AccordionBody>
          <div className="min-w-0 py-2 p-0 px-0 grid grid-cols-2 gap-3">
            {soldproductsOptions &&
              soldproductsOptions.length > 0 &&
              soldproductsOptions.map((soldProduct, index) => {
                return (
                  <div className="p-0" key={index}>
                    <label
                      htmlFor={soldProduct?.text}
                      className="flex w-full cursor-pointer items-center"
                    >
                      <Checkbox
                        value={{ min: soldProduct?.min, max: soldProduct?.max }}
                        onChange={(event) =>
                          handleSoldProductsChange(
                            event,
                            soldProduct?.min,
                            soldProduct?.max
                          )
                        ...
                        checked={
                          selectedFilters?.soldProducts &&
                          selectedFilters?.soldProducts.some(
                            (SoldProducts) =>
                              SoldProducts.min === soldProduct?.min &&
                              SoldProducts.max === soldProduct?.max
                          )
                        }
                        className="w-4 h-4"
                        labelProps={{ className: "pl-2" }}
                      ...
                      >
                        <Checkbox
                          ...
                          checked={
                            ...
                            selectedFilters?.characteristics?.some(
                              (selectedCharacteristic) =>
                                selectedCharacteristic === trend?.chars_id
                            )
                          }
                          className="w-4 h-4"
                          labelProps={{ className: "pl-2" }}
