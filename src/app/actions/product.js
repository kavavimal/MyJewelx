...
export const searchProducts = async (data) => {
  ...
  try {
    ...
    if (
      data?.price &&
      typeof data?.price?.min === "number" &&
      typeof data?.price?.max === "number" &&
      Number(data.price.min) <= Number(data.price.max)
    ) {
      products = products.filter((product) => {
        const isPriceWithinRange = product.variations.some((variation) => {
          return (
            Number(variation.selling_price) >= Number(data.price.min) &&
            Number(variation.selling_price) <= Number(data.price.max)
          );
        });
